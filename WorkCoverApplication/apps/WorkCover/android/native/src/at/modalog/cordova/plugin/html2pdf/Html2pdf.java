...
public class Html2pdf extends CordovaPlugin {
	...
	private String publicTmpDir = ".at.modalog.cordova.plugin.html2pdf"; // prepending
																			// a
																			// dot
																			// "."
																			// would
		...
		try {
			if (action.equals("create")) {
				if (showWebViewForDebugging) {
					...
					Log.v(LOG_TAG,
							"Html start:" + args.getString(0).substring(0, 30));
					Log.v(LOG_TAG,
							"Html end:"
									+ args.getString(0).substring(
											args.getString(0).length() - 30));
				}
				...
				cordova.getActivity().runOnUiThread(new Runnable() {
					public void run() {
						...
						if (true) {
							...
							/*
							 * Kitkat pdf creation by using the android print
							 * framework (Android >= 4.4)
							 */
							...
							page.getSettings().setLayoutAlgorithm(
							...
							// self.cordova.getActivity().addContentView(webView,
							// new
							...
							page.setWebViewClient(new WebViewClient() {
								...
								public void onPageFinished(WebView view,
									...
									PrintManager printManager = (PrintManager) self.cordova
											.getActivity().getSystemService(
													Context.PRINT_SERVICE);

									// Get a print adapter instance
									PrintDocumentAdapter printAdapter = view
									...
									// builder.setResolution(new
									// PrintAttributes.Resolution("default",
									// self.tmpPdfName, 600, 600));
									...
									// Log.d("abcd","self.tmpPdfName"+self.tmpPdfName);
									printManager.print(filePdf.getName(),
											printAdapter, builder.build());

								}
							});

							// Reverse engineer base url (assets/www) from the
							// cordova webView url
							...
							page.loadDataWithBaseURL(baseURL, content,
	...
	/**
	 * Loads the html content into a WebView, saves it as a single multi page
	 * pdf file and calls startPdfApp() once it´s done.
	 */
				...
				new Handler().postDelayed(new Runnable() {
					...
					public void run() {
						...
						if (b != null) {
							...
							Intent pdfViewIntent = new Intent(
									Intent.ACTION_VIEW);
							pdfViewIntent.setDataAndNormalize(Uri
									.fromFile(tmpFile));
							...
							// start the pdf viewer app (trigger the pdf view
							// intent)
							PluginResult result;
...
//							}
		...
		/**
		 * We make it this small on purpose (is resized after page load has
		 * finished). Making it small in the beginning has some effects on the
		 * html <body> (body width will always remain 100 if not set
		 * explicitly).
		 */
		if (!showWebViewForDebugging) {
			ctx.addContentView(page, new ViewGroup.LayoutParams(100, 100));
		} else {
			ctx.addContentView(page, new ViewGroup.LayoutParams(
	...
	/**
	 * Takes a WebView and returns a Bitmap representation of it (takes a
	 * "screenshot").
	 * 
	 * @param WebView
	 * @return Bitmap
	...
	Bitmap getWebViewAsBitmap(WebView view) {
		...
		Log.v(LOG_TAG, "Html2Pdf.getWebViewAsBitmap -> Content width: " + width
				+ ", height: " + height);
	...
	/**
	 * Slices the screenshot into pages, merges those into a single pdf and
	 * saves it in the public accessible /sdcard dir.
	 */
	private File saveWebViewAsPdf(Bitmap screenshot) {
		try {
			...
			double pageWidth = PageSize.A4.getWidth() * 0.99; // width of the
																// image is 85%
																// of the page
			double pageHeight = PageSize.A4.getHeight() * 0.99; // max height of
																// the image is
																// 80% of the
																// page
			double pageHeightToWithRelation = pageHeight / pageWidth; // e.g.:
																		// 1.33
																		// (4/3)
			...
			int sliceHeight = (int) Math.round(sliceWidth
					* pageHeightToWithRelation);
			while (totalSize > currPos && currPageCount < 100) // max 100 pages
			{
				...
				// slice bitmap
				currPage = Bitmap.createBitmap(screenshot, 0, currPos,
						sliceWidth,
						(int) Math.min(sliceHeight, totalSize - currPos));

				// save page as png
				stream = new FileOutputStream(new File(dir, "pdf-page-"
						+ currPageCount + ".png"));
			...
			File filePdf = new File(sdCard.getAbsolutePath() + "/"
					+ this.tmpPdfName);
			Log.d("abcd", filePdf.toString());
			// change the output name of the pdf here
			// create dirs if necessary
			if (this.tmpPdfName.contains("/")) {
				File filePdfDir = new File(filePdf.getAbsolutePath().substring(
						0, filePdf.getAbsolutePath().lastIndexOf("/"))); // get
																			// the
																			// dir
																			// portion
				filePdfDir.mkdirs();
